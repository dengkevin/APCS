...
public class Ch25Exercises
{
    ...
    {
        int nodes = (Integer)completeTree[0];
        int leaves = ( nodes + 1 ) / 2;
        nodes -= leaves;
        completeTree[0] = nodes;
    }
    ...
    private void toArray( TreeNode node, Object[] values, int i )
    {
        ...
        toArray( node.getRight(), values, i + 1 );
    }
    ...
    public boolean isHeap( Integer[] items, int numNodes )
    {
        ...
        while ( index * 2 < numNodes && index * 2 + 1 < numNodes )
        {
            int currentParent = items[index];
            if ( items[index * 2] < currentParent || items[index * 2 + 1] < currentParent )
            {
                return false;
            }
            index++;
    ...
    public void reheapDown( Integer[] items, int numNodes )
    {
        ...
        while ( 2 * index <= numNodes && 2 * index + 1 <= numNodes )
        {
        ...
        if (2 * index <= numNodes && items[2 * index] < items[index])
        {
            pos = 2 * index;
            temp = items[index];
            items[index] = items[pos];
