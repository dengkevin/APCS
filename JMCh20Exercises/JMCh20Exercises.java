...
public class JMCh20Exercises<E>
{
    ...
    public ListNode<E> reverseList( ListNode<E> head )
    {
        ...
        {
            newHead = new ListNode<E>(head.getValue(), null);
            for (node = head.getNext(); node != null; node = node.getNext())
            {
                ListNode<E> add = new ListNode<E>(node.getValue(), null);
                add.setNext(newHead);
                newHead = add;
            }
        }
    ...
    public ListNode<String> concatenateStrings( ListNode<String> head )
    {
        ...
        else
        {
            ...
            ListNode<String> node1 = newHead;
            String stringNode = "";
            
            for (ListNode<String> node = head; node != null; node = node.getNext())
            {
                stringNode = stringNode + node.getValue();
                
                if (node != head)
                {
                    node1.setNext(new ListNode<String>(stringNode, null));
                    node1 = node1.getNext();
                }
            }
    ...
    {
        ListNode<String> newHead = new ListNode<String>(s, null);
        ...
        {            
            for (ListNode<String> node = head; node != null; node = node.getNext())
            {
                ...
                else if (node.getNext() == null && node.getValue().compareToIgnoreCase(s) < 0)
                {
                    node.setNext(newHead);
                    break;
                }
                else if (node.getValue().compareToIgnoreCase(s) < 0 && node.getNext().getValue().compareToIgnoreCase(s) > 0)
                {
                    newHead.setNext(node.getNext());
                    node.setNext(newHead);
                    break;
                }
                else if (head.getValue().compareToIgnoreCase(s) > 0)
                {
    ...
    public static void sort( ListNode<String> head )
    {
        ...
        for (ListNode<String> node = head; node != null; node = node.getNext())
        {
            ...
            for (ListNode<String> node1 = min.getNext(); node1 != null; node1 = node1.getNext())
            {
                if (node1.getValue().compareTo(min.getValue()) < 0)
                {
                    min = node1;
                }
            }
    ...
    public static ListNode2<Integer> removeMax( ListNode2<Integer> head )
    {
        ...
        for (ListNode2<Integer> node = head; node != null; node = node.getNext())
        {
