...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        {
            Message remv = mailbox.remove();
            
            if (remv.getText().startsWith("need"))
            {
                if (this.countPossessions(remv.getText().substring(5)) > 1)
                {
                    announce("have " + remv.getText().substring(5));
                }
            }
            else if (remv.getText().startsWith("have"))
            {
                if (this.countPossessions(remv.getText().substring(5)) == 0)
                {
                    announce("ship " + remv.getText().substring(5));
                }
            }
            else if (remv.getText().startsWith("ship"))
            {
                if (this.countPossessions(remv.getText().substring(5)) > 1)
                {
                    if (remv.getSender().receiveItem(this, 
                        remv.getText().substring(5)))
                    {
                        myPossessions.remove(remv.getText().substring(5));
                    }
                }
            }
        }
